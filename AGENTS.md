...
1. **Mockup-first, static data.** Do not introduce Supabase, Claude API calls, or any other live backend unless the user explicitly asks. The reducer in [src/hooks/useGroupState.ts](src/hooks/useGroupState.ts) is the single intentional integration seam — replace it later, not now.
...
3. **No router.** The app uses state-based view switching (`state.joined` in `App.tsx`, `useState<TabId>` in `GroupScreen.tsx`). This keeps deploys trivial on both Vercel and GitHub Pages. Don't add `react-router-dom` unless the user asks for deep links.
4. **Bottom nav: only "Group" is enabled.** [src/layout/BottomNav.tsx](src/layout/BottomNav.tsx) deliberately disables Home / Schedule / Scan / Profile. The user picked this in planning to keep demo focus crystal clear. Don't "helpfully" make them clickable.
...
6. **Real event data only.** All event IDs in [src/data/mockGroup.ts](src/data/mockGroup.ts) reference real entries in `events.json` (515 sessions, scraped). When adding a new mock wishlist entry or AI match, look up a real event ID first — the cards pull title, time, location, speakers, and description from the JSON.
...
flowchart TD
    ...
    App --> Provider[GroupProvider]
    Provider --> Router{state.joined?}
    Router -->|no| Join[JoinScreen]
    Router -->|yes| Group[GroupScreen]
    Group --> Header[GroupHeader]
    Group --> Tabs[TabBar]
    Group --> Body{active tab}
    Group --> Nav[BottomNav]
    ...
    Status --> QSB
    Card --> Hook[useGroupState]
    QSB --> Hook
    Status --> Hook
    Map --> FloorPlan[FloorPlan SVG]
    Map --> Hook
    Hook --> Storage[(localStorage<br/>wsv-group-state-v1)]
    Hook --> MockData[mockGroup.ts]
    Card --> EventsData[events.json]
```

**Data flow:** mock seed data (`mockGroup.ts`) → `useReducer` in `GroupProvider` → context → components. Mutations go back through `useGroupState()` actions (`join`, `leave`, `broadcast`, `toggleWishlist`, `moveUser`, `refreshAiMatches`). The reducer auto-syncs to `localStorage` after every change.
...
| ----------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| Change a member's name, avatar, or interests          | `MEMBERS` in [src/data/mockGroup.ts](src/data/mockGroup.ts)                                                                            |
| Add / remove events from the curated wishlist         | `WISHLIST` in [src/data/mockGroup.ts](src/data/mockGroup.ts) — use real IDs from `events.json`                                         |
| Change a status preset, its label or icon             | `STATUS_PRESETS` in [src/data/mockGroup.ts](src/data/mockGroup.ts) and `ICONS` map in [QuickStatusBar.tsx](src/features/group/QuickStatusBar.tsx) + [StatusTab.tsx](src/features/group/StatusTab.tsx) |
| Reposition pre-set member pins on the map             | `INITIAL_MAP_POSITIONS` in [src/data/mockGroup.ts](src/data/mockGroup.ts) (0–100 % of the SVG viewBox)                                 |
| Add new named zones for "Pick zone" sheet             | `MAP_ZONES` in [src/data/mockGroup.ts](src/data/mockGroup.ts)                                                                          |
| Add / tweak AI match recommendations                  | `AI_MATCH_SETS` in [src/data/mockGroup.ts](src/data/mockGroup.ts) — two sets, "Refresh Matches" rotates between them                   |
| Change the join code                                  | `GROUP_CODE` in [src/data/mockGroup.ts](src/data/mockGroup.ts) and the placeholder copy in [JoinScreen.tsx](src/screens/JoinScreen.tsx) |
| Recolour a summit format's pill                       | `FORMAT_COLORS` in [src/utils/events.ts](src/utils/events.ts)                                                                          |
| Adjust the venue floor plan illustration              | SVG paths in [src/features/group/FloorPlan.tsx](src/features/group/FloorPlan.tsx)                                                      |
...
- **Member roster.** 5 logical members live in `MEMBERS`: `you`, `daniel`, `jessica`, `ramesh`, `zohaib`. `OTHER_MEMBER_IDS` is the 4 friends excluding `you`. The header displays "N / 6 MEMBERS" using `MEMBER_LIMIT = 6`, with the 4 friends shown as avatar circles and a "+" to invite more.
- **The user is "You", not a fifth profile.** "You" appears in feeds and on the map, but the avatar row in `GroupHeader` only shows the 4 friends. Overlap counts in `EventCard` are denominated `N / 4` (other members), with a separate filled-bookmark icon to indicate whether `you` are also going. Don't conflate these two signals.
...
## 7. State model (`useGroupState`)

The reducer in [src/hooks/useGroupState.ts](src/hooks/useGroupState.ts) holds:
...
Actions:
- `JOIN` / `LEAVE` — `LEAVE` re-seeds from `mockGroup.ts` (effective reset for the demo)
...
The storage key is `wsv-group-state-v1`. **Bump the suffix if you change the shape** so old localStorage doesn't break new builds.

`GroupProvider` is the only sane way to mount the app. `useGroupState()` throws if called outside it.
...
### Add a new tab to the Group screen
1. Add an entry to `TABS` in [src/features/group/TabBar.tsx](src/features/group/TabBar.tsx) and extend the `TabId` union.
2. Create a `NewTab.tsx` component in `src/features/group/`.
3. Wire it into the `tab === "..."` switch in [src/screens/GroupScreen.tsx](src/screens/GroupScreen.tsx).
...
2. Replace `currentAiMatches` derivation in [src/hooks/useGroupState.ts](src/hooks/useGroupState.ts) with an async fetch keyed off `state.wishlist` and the member tags.
3. Keep the `AI_MATCH_SETS` fallback for offline/demo mode — the user values offline-resilience.

### Make the prototype multiplayer
The reducer in `useGroupState.ts` is intentionally the only place that needs to change. Swap the storage adapter for a Supabase channel that mirrors `GroupState`. UI components don't need to know.

### Add a new status preset
1. Append to `STATUS_PRESETS` in [src/data/mockGroup.ts](src/data/mockGroup.ts) (and extend the `StatusPresetId` union).
2. Add icon + tint entries in `ICONS` ([QuickStatusBar.tsx](src/features/group/QuickStatusBar.tsx)) and `ICON_MAP` + `TINT_MAP` ([StatusTab.tsx](src/features/group/StatusTab.tsx)).
...
Drop images into `public/avatars/`, then set each `Member.avatar` in `mockGroup.ts` to `"./avatars/daniel.jpg"` etc. The relative path works under `base: "./"`.
...
src/
  main.tsx                            React root
  App.tsx                             Provider + Join/Group view switch
  index.css                           Tailwind + .card / .pill / .tap
  vite-env.d.ts
  data/
    events.json                       515 scraped sessions (bundled)
    mockGroup.ts                      ALL demo content lives here
  hooks/
    useGroupState.ts                  Reducer + Context + localStorage
  utils/
  ...
  screens/
    ...
    GroupScreen.tsx                   Header + tab content + nav shell
  features/group/
    GroupHeader.tsx                   Title, Leave, join code card, members row
